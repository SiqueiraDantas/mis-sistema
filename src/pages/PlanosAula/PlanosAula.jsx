...
function ModalPlano({ plano, turmaId, professorId, onClose, onSalvo }) {
  ...
  async function gerarPlanoMensal() {
    if (!form.conteudo.trim()) { setErro('Preencha o conteudo primeiro para gerar o plano mensal'); return }
    ...
    try {
      ...
      const response = await fetch('https://rvmucdufiaalckacmlmi.supabase.co/functions/v1/gerar-plano', {
        method: 'POST',
        headers: {
      ...
      <div className="min-h-full flex items-start justify-center p-3 py-6">
        <div className="w-full max-w-2xl bg-mis-bg2 border border-mis-borda rounded-xl2 animate-fade-in">
          <div className="flex items-center justify-between p-4 border-b border-mis-borda">
            <h2 className="text-sm font-bold text-mis-texto">{editando ? 'Editar Plano de Aula' : 'Novo Plano de Aula'}</h2>
          ...
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              <textarea className="mis-input min-h-[100px] resize-y" rows={4}
            ...
            <div>
              ...
              <textarea className="mis-input min-h-[100px] resize-y" rows={4}
                ...
                value={form.metodologia} onChange={e => set('metodologia', e.target.value)}/>
            </div>
            <div>
              <label className="mis-label">Necessidades tecnicas / Materiais</label>
              <textarea className="mis-input min-h-[80px] resize-y" rows={3}
                placeholder="Ex: Instrumento, metodo Arban, espaco adequado..."
                value={form.materiais} onChange={e => set('materiais', e.target.value)}/>
            </div>
            <div className="border border-mis-borda rounded-lg p-3 bg-mis-bg3">
              ...
              <textarea className="mis-input min-h-[80px] resize-y" rows={3}
                ...
                value={form.plano_mensal} onChange={e => set('plano_mensal', e.target.value)}/>
            </div>
            {erro && (
...
function ModalExcluir({ plano, onClose, onConfirmar }) {
  const [loading, setLoading] = useState(false)
  async function confirmar() { setLoading(true); await onConfirmar(); setLoading(false) }
  return createPortal(
        ...
        <div className="w-full max-w-lg bg-mis-bg2 border border-mis-borda rounded-xl2 animate-fade-in">
          ...
          <div className="p-4 space-y-3">
            ...
            <div>
              ...
              <textarea className="mis-input min-h-[140px] resize-y" rows={6}
                placeholder="Descreva sua formacao e experiencia musical..."
                value={form.mini_curriculo} onChange={e => set('mini_curriculo', e.target.value)}/>
            </div>
          </div>
          <div className="flex gap-2 p-4 border-t border-mis-borda">
            <button onClick={onClose} className="btn-secondary px-4 py-2 text-sm">Cancelar</button>
...
export default function PlanosAula() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (minhasTurmas?.length > 0) setTurmaSelecionada(minhasTurmas[0].id)
    }
    init()
  }, [user, isDiretor])
  ...
  const buscarPlanos = useCallback(async () => {
    ...
    const { data, error } = await supabase.from('planos_aula')
      .select('*').eq('turma_id', turmaSelecionada)
    ...
    <div className="animate-fade-in">
      <div className="flex items-start justify-between mb-5 gap-2">
        <div>
          ...
          <p className="text-mis-texto2 text-sm mt-1">{planos.length} aula{planos.length !== 1 ? 's' : ''} · {MESES[mesFiltro]} {ANO_ATUAL}</p>
        </div>
      ...
      <div className="mis-card mb-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select className="mis-input text-sm" value={turmaSelecionada} onChange={e => setTurmaSelecionada(e.target.value)}>
      ...
      {loading ? (
        ...
        <div className="mis-card flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-mis-texto2 text-sm mb-4">{MESES[mesFiltro]} - {turmaAtual?.nome}</p>
              ...
              <div key={plano.id} className="mis-card hover:border-mis-texto/20 transition-all">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-mis-texto mb-1">{turmaAtual?.oficinas?.nome || turmaAtual?.nome}</p>
      ...
      {(modalNovo || planoEditar) && (
        <ModalPlano plano={planoEditar} turmaId={turmaSelecionada} professorId={user.id}
          onClose={() => { setModalNovo(false); setPlanoEditar(null) }}
          onSalvo={() => { setModalNovo(false); setPlanoEditar(null); buscarPlanos() }}/>
      )}
      {planoExcluir && (
        <ModalExcluir plano={planoExcluir} onClose={() => setPlanoExcluir(null)} onConfirmar={() => excluir(planoExcluir)}/>
      )}
      {modalCurriculo && perfil && (
        <ModalCurriculo perfil={perfil} onClose={() => setModalCurriculo(false)}
          onSalvo={() => {
